...
class CharucoDetector:
    ...
    def draw_overlay(self, img, det):
        ...
        # 検出情報のテキスト表示
        text = f"Markers: {len(det['ids'])} | " f"ChArUco Corners: {len(det['ch_ids'])}"
    ...
    def process_camera(self, camera_name):
        ...
        for i, img_path in enumerate(image_files):
            ...
            if det is not None:
                ...
                print(f"Failed to detect: " f"{img_path.name}")
        ...
        print(f"  Success: {success_count} " f"({success_rate:.1f}%)")
        print(
            f"  Failed: {total_frames - success_count} " f"({100 - success_rate:.1f}%)"
        )
        ...
        if success_count > 0:
            ...
            for frame_id, detection in enumerate(successful_detections):

                frame_data = {
                    "frame_id": frame_id,
                    "image_name": detection["image_name"],
                    "ch_corners": detection["ch_corners"].reshape(-1, 2).tolist(),
                    "ch_ids": detection["ch_ids"].flatten().tolist(),
                    "aruco_corners": [
                        corner.reshape(-1, 2).tolist()
                        for corner in detection["corners"]
                    ],
                    "aruco_ids": detection["ids"].flatten().tolist(),
                }
            ...
            print(f"Saved detection results to: " f"{detection_file}")
            ...
            print(f"Saved statistics to: " f"{stats_file}")
    ...
    def process_all_cameras(self):
        ...
        camera_dirs = sorted(
            ...
            if (path.is_dir() and path.name.startswith("camera"))
        )

        if not camera_dirs:
            print(f"No camera folders found in " f"{self.captured_images_dir}")
            return all_stats
        ...
        print(f"Cameras processed: " f"{len(all_stats)}")

        print(f"Total frames: " f"{total_frames}")

        print(f"Total success: " f"{total_success} " f"({overall_rate:.1f}%)")
        ...
        print(f"\nSaved overall statistics to: " f"{overall_stats_file}")
...
def main():
    ...
    # 検出器の初期化
    detector = CharucoDetector(calibration_config_path="calibration_config.yaml")
    ...
    print(f"  - Detection cache: " f"{detector.detection_cache_dir}")

    print(f"  - Overlay images: " f"{detector.output_dir}")
