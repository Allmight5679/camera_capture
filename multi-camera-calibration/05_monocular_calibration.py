...
class MonocularCalibrator:
    ...
    def load_detections(self, camera_name):
        ...
        print(f"Loading detections from: " f"{detection_file}")
        ...
        for frame in data["frames"]:
            ...
            corners_array = np.array(frame["ch_corners"], dtype=np.float32).reshape(
                -1, 1, 2
            )
        ...
        print(f"  Loaded " f"{len(ch_corners)} frames")

        print(f"  Image size: " f"{img_size}")

        return {"ch_corners": ch_corners, "ch_ids": ch_ids, "img_size": img_size}
    ...
    def calibrate_camera(self, detections, camera_name):
        ...
        print(f"  Number of frames: " f"{len(ch_corners)}")
        ...
        print(f"  RMS reprojection error: " f"{ret:.4f} px")
        ...
        print(
            f"  Status: " f"{color}{status}\033[0m " f"(Target: {self.target_rms} px)"
        )
        ...
        print(f"  Focal length: " f"fx={fx:.2f}, fy={fy:.2f}")

        print(f"  Principal point: " f"cx={cx:.2f}, cy={cy:.2f}")

        print(f"  Aspect ratio: " f"{fx / fy:.4f}")
    ...
    def compute_per_frame_errors(self, calibration_result, detections):
        ...
        for i, (rvec, tvec, ch_corners, ch_ids) in enumerate(
            zip(rvecs, tvecs, ch_corners_list, ch_ids_list)
        ):

            obj_points = self.board.getChessboardCorners()[ch_ids.ravel()]

            projected_points, _ = cv2.projectPoints(obj_points, rvec, tvec, K, dist)

            errors = np.linalg.norm(
                ch_corners.reshape(-1, 2) - projected_points.reshape(-1, 2), axis=1
            )
        ...
        all_mean_errors = [frame_error["mean_error"] for frame_error in frame_errors]
        ...
        print("  Overall error statistics (px):")
        ...
        print(f"    Median: " f"{overall_stats['median']:.4f}")

        return {"frame_errors": frame_errors, "overall": overall_stats}
    ...
    def save_calibration_result(self, calibration_result, error_stats):
        ...
        img_size_list = [
            int(calibration_result["img_size"][0]),
            int(calibration_result["img_size"][1]),
        ]
        ...
        print(f"\nSaved calibration results to: " f"{json_file}")
    ...
    def plot_error_histogram(self, error_stats, camera_name):
        ...
        mean_errors = [
            frame_error["mean_error"] for frame_error in error_stats["frame_errors"]
        ]
        ...
        plt.axvline(
            ...
            color="r",
            linestyle="--",
            label=(f"Mean: " f"{error_stats['overall']['mean']:.4f} px"),
        )
        ...
        plt.axvline(
            ...
            color="g",
            linestyle="--",
            label=(f"Median: " f"{error_stats['overall']['median']:.4f} px"),
        )
        ...
        plt.title(f"Reprojection Error Distribution " f"- {camera_name}", fontsize=14)
        ...
        print(f"Saved error histogram to: " f"{histogram_file}")

    def visualize_undistortion(
        self, calibration_result, camera_name, save_all=True, num_samples=3
    ):
        """
        歪み補正の効果を可視化
        """
        ...
        captured_images_dir = (
            Path(self.config["paths"]["captured_images"]) / camera_name
        )
        ...
        if save_all:
            ...
            print(f"  Processing all " f"{len(files_to_process)} images...")
            ...
            print(f"  Processing " f"{len(files_to_process)} " f"sample images...")
        ...
        for i, img_file in enumerate(files_to_process):
            ...
            if img is None:
                print(f"  Warning: Failed to load " f"{img_file.name}")
                continue
            ...
            comparison_file = camera_undistort_dir / (
                f"{img_file.stem}" f"_comparison.png"
            )

            cv2.imwrite(str(comparison_file), combined)

            undistorted_file = camera_undistort_dir / (
                f"{img_file.stem}" f"_undistorted.png"
            )
            ...
            if (i + 1) % 5 == 0 or (i + 1 == len(files_to_process)):
                print(f"  Processed " f"{i + 1}/" f"{len(files_to_process)} " f"images")
    ...
    def verify_reproducibility(self, camera_name):
        ...
        print(f"\nVerifying reproducibility " f"for {camera_name}...")

        json_file = self.calibration_dir / f"calibration_{camera_name}.json"

        if not json_file.exists():
            print(f"  Error: Calibration file " f"not found: {json_file}")
            return False
        ...
        print(f"  Loaded RMS from file: " f"{loaded_rms:.4f} px")
        ...
        print("  ✓ Reproducibility check passed: " "Data successfully loaded from JSON")
    ...
    def process_all_cameras(self):
        ...
        print(f"Started at: " f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        print(f"{'=' * 60}")

        # detection_cacheからカメラを自動検出
        detection_files = sorted(
            self.detection_cache_dir.glob("detections_camera*.json")
        )

        if not detection_files:
            print(f"No detection files found in " f"{self.detection_cache_dir}")

            return all_results

        camera_names = [
            detection_file.stem.replace("detections_", "")
        ...
        for camera_name, result in all_results.items():
            ...
            print(f"{status} " f"{camera_name}: " f"RMS = " f"{result['rms']:.4f} px")

        print(f"\n{'=' * 60}")

        print(f"Finished at: " f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        print(f"{'=' * 60}\n")

        print("Results saved to:")

        print(f"  - Calibration data: " f"{self.calibration_dir}")

        print(f"  - Undistortion samples: " f"{self.undistort_dir}")
