...
class DetectionPacket:
    frame: FramePacket
    detections: List[List['HandDetection']]


@dataclass
class ReconstructionPacket:
    frame: FramePacket
    detections: List[List['HandDetection']]
    hands: List['Hand3DReconstruction']
...
_DETECTION_WORKER_INSTANCE: Optional[MultiCameraHandDetector] = None


def _detection_worker_initializer(max_num_hands: int, min_detection_confidence: float, min_tracking_confidence: float) -> None:
    """Initializer for detection worker processes/threads."""
    global _DETECTION_WORKER_INSTANCE
    _DETECTION_WORKER_INSTANCE = MultiCameraHandDetector(
        max_num_hands=max_num_hands,
        min_detection_confidence=min_detection_confidence,
        min_tracking_confidence=min_tracking_confidence
    )
...
def _detect_frame_bundle_worker(frames: List[np.ndarray]) -> List[List[Dict[str, Any]]]:
    """Run MediaPipe detection for a bundle of frames (worker-side)."""
    if _DETECTION_WORKER_INSTANCE is None:
        raise RuntimeError("Detection worker not initialized")

    bundle: List[List[Dict[str, Any]]] = []
    for frame in frames:
        detections = _DETECTION_WORKER_INSTANCE.detect(frame)
...
class MultiCameraHand3DSystem:
    ...
    def __init__(self, 
        ...
        async_cfg = self.hand3d_params.get('async_pipeline', {})
        self._frame_queue_maxsize = int(async_cfg.get('frame_queue_maxsize', 2))
        self._detection_queue_maxsize = int(async_cfg.get('detection_queue_maxsize', 2))
        self._recon_queue_maxsize = int(async_cfg.get('recon_queue_maxsize', 1))
        self._capture_timeout_ms = int(async_cfg.get('capture_timeout_ms', 1000))
        self._visual_interval_sec = float(async_cfg.get('visual_interval_sec', 1.0 / 30.0))
        self._queue_get_timeout_sec = float(async_cfg.get('queue_get_timeout_sec', 0.1))

        # Executors and async bookkeeping
        self._capture_executor: Optional[ThreadPoolExecutor] = None
        self._detection_executor: Optional[Any] = None
        self._detection_executor_mode: str = hand_det_cfg.get('executor', 'process')
        self._reconstruction_executor: Optional[ThreadPoolExecutor] = None
    ...
    def capture_frames(self) -> List[np.ndarray]:
        ...
        for cam in self.cameras:
            img = cam.capture_image(self._capture_timeout_ms)
            if img is None:
                raise RuntimeError("Failed to capture frame")
            frames.append(img)
        return frames

    def _ensure_capture_executor(self) -> None:
        """Initialize capture executor if needed."""
    ...
    def _ensure_detection_executor(self) -> None:
        """Initialize detection executor based on configuration."""
        if self._detection_executor_mode == 'inline':
        ...
        detection_cfg = self.hand3d_params['hand_detection']
        workers = detection_cfg.get('executor_workers', len(self.cameras))
        workers = max(1, workers)
        mode = self._detection_executor_mode.lower()

        if mode == 'process':
            try:
                self._detection_executor = ProcessPoolExecutor(
                    max_workers=workers,
                    initializer=_detection_worker_initializer,
                    initargs=(
                        detection_cfg['max_num_hands'],
                        detection_cfg['min_detection_confidence'],
                        detection_cfg['min_tracking_confidence']
                    )
                )
                return
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    "Process detection executor initialization failed (%s). Falling back to inline execution.",
                    exc
                )
                self._detection_executor = None
                self._detection_executor_mode = 'inline'
                return

        if mode == 'thread':
            # Initialize shared detector instance for thread workers
            _detection_worker_initializer(
                detection_cfg['max_num_hands'],
                detection_cfg['min_detection_confidence'],
                detection_cfg['min_tracking_confidence']
            )
    ...
    def _ensure_reconstruction_executor(self) -> None:
        """Initialize reconstruction executor if needed."""
        if self._reconstruction_executor is None:
            workers = self.hand3d_params['reconstruction'].get('executor_workers', 2)
            workers = max(1, workers)
            self._reconstruction_executor = ThreadPoolExecutor(
                max_workers=workers,
    ...
    def _shutdown_executors(self) -> None:
        ...
        executors = [self._capture_executor, self._detection_executor, self._reconstruction_executor]
        for executor in executors:
            ...
            try:
                ...
                logger.debug("Executor shutdown error: %s", exc)
        self._capture_executor = None
        self._detection_executor = None
        self._reconstruction_executor = None

    @staticmethod
    def _deserialize_detections(serialized: List[List[Dict[str, Any]]]) -> List[List['HandDetection']]:
        """Convert serialized detection payload into HandDetection objects."""
        detections: List[List[HandDetection]] = []
        for cam_data in serialized:
            cam_detections: List[HandDetection] = []
            for det in cam_data:
                landmarks = np.asarray(det['landmarks'], dtype=np.float32)
                scores = np.asarray(det['scores'], dtype=np.float32)
                cam_detections.append(HandDetection(landmarks, scores, str(det['handedness'])))
            detections.append(cam_detections)
        return detections

    @staticmethod
    async def _put_with_overwrite(queue: asyncio.Queue, item: Any) -> None:
        """Put item into queue, discarding oldest if queue is full."""
        if queue.full():
            with contextlib.suppress(asyncio.QueueEmpty):
                queue.get_nowait()
        await queue.put(item)

    async def _capture_loop(self, frame_queue: asyncio.Queue, stop_event: asyncio.Event) -> None:
        """Capture frames asynchronously and push to queue."""
        self._ensure_capture_executor()
        ...
        try:
            while not stop_event.is_set():
                capture_tasks = [
                    loop.run_in_executor(self._capture_executor, cam.capture_image, self._capture_timeout_ms)
                    for cam in self.cameras
                ]
                ...
                for idx, result in enumerate(results):
                    if isinstance(result, Exception):
                        logger.warning("Capture error on camera %s: %s", camera_names[idx], result)
                        skip_frame = True
                        break
                    if result is None:
                        logger.warning("Capture timeout on camera %s", camera_names[idx])
                        skip_frame = True
                        break
                    frames.append(cast(np.ndarray, result))
                ...
                await self._put_with_overwrite(frame_queue, packet)
                frame_id += 1
    ...
    async def _detection_loop(
        ...
        self._ensure_detection_executor()
        loop = asyncio.get_running_loop()
        ...
        try:
            while True:
                if stop_event.is_set() and frame_queue.empty():
                    break
                try:
                    frame_packet: FramePacket = await asyncio.wait_for(
                        frame_queue.get(),
                ...
                if self._detection_executor_mode == 'inline':
                    detections_per_camera = [
                        self.hand_detector.detect(frame) for frame in frame_packet.frames
                    ]
                else:
                    raw = await loop.run_in_executor(
                        self._detection_executor,  # type: ignore[arg-type]
                        _detect_frame_bundle_worker,
                        frame_packet.frames
                    )
                    detections_per_camera = self._deserialize_detections(raw)

                packet = DetectionPacket(frame=frame_packet, detections=detections_per_camera)
                await self._put_with_overwrite(detection_queue, packet)
        except asyncio.CancelledError:
            logger.debug("Detection loop cancelled")
        except Exception as exc:  # noqa: BLE001
    ...
    async def _reconstruction_loop(
        self,
        detection_queue: asyncio.Queue,
        recon_queue: asyncio.Queue,
        stop_event: asyncio.Event
    ) -> None:
        """Perform 3D reconstruction asynchronously."""
        self._ensure_reconstruction_executor()
        loop = asyncio.get_running_loop()

        try:
            while True:
                if stop_event.is_set() and detection_queue.empty():
                    break
                try:
                    detection_packet: DetectionPacket = await asyncio.wait_for(
                        detection_queue.get(),
                        timeout=self._queue_get_timeout_sec
                    )
                except asyncio.TimeoutError:
                ...
                self.frame_count = detection_packet.frame.frame_id
                worker = functools.partial(
                    self.reconstruct_hands,
                    detection_packet.frame.frames,
                    detection_packet.detections
                )
                hands = await loop.run_in_executor(
                    self._reconstruction_executor,
                    worker
                )
                self.frame_count = detection_packet.frame.frame_id + 1

                recon_packet = ReconstructionPacket(
                    frame=detection_packet.frame,
                    detections=detection_packet.detections,
                    hands=hands,
                    latency_sec=max(0.0, time.time() - detection_packet.frame.timestamp)
                )
                await self._put_with_overwrite(recon_queue, recon_packet)
        except asyncio.CancelledError:
            logger.debug("Reconstruction loop cancelled")
        except Exception as exc:  # noqa: BLE001
            logger.exception("Reconstruction loop error: %s", exc)
            stop_event.set()
    ...
    def _reset_visual_state(self) -> None:
        """Reset dynamic visualization state."""
        self.static_geometries_initialized = False
        self.hand_point_actors.clear()
        self.hand_line_actors.clear()
        self.ghost_point_actors.clear()
        self.ghost_line_actors.clear()

    def _log_frame_stats(self, packet: ReconstructionPacket) -> None:
        """Log per-frame statistics once per frame."""
        detection_count = sum(len(dets) for dets in packet.detections)
        latency_ms = packet.latency_sec * 1000.0
        fps = 0.0
        if self._last_frame_timestamp is not None:
            dt = packet.frame.timestamp - self._last_frame_timestamp
            if dt > 1e-6:
                fps = 1.0 / dt
        self._last_frame_timestamp = packet.frame.timestamp
        logger.info(
            "Frame %d: Raw detections=%d, Reconstructed=%d hands, latency=%.1f ms, FPS=%.1f",
    ...
    async def _save_packet_async(self, packet: ReconstructionPacket) -> None:
        """Asynchronously save current frame snapshot."""
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, self.save_frame, packet.frame.frames, packet.hands)

    def _process_visual_packet(
        self,
        ...
        self.visualize_2d(packet.frame.frames, packet.hands, packet.detections)

        if show_3d and plotter is not None:
            self.update_hand_geometries(plotter, packet.hands)
            self.update_ghost_hand_geometries(plotter)
            if packet.hands:
                center = self.calculate_hands_center(packet.hands)
                self.update_camera_view(plotter, center)
            plotter.render()

        if packet.frame.frame_id != self._last_rendered_frame_id:
            self._last_rendered_frame_id = packet.frame.frame_id
            self.total_reconstructions += len(packet.hands)
            self._log_frame_stats(packet)

    async def _visualization_loop(
        self,
        recon_queue: asyncio.Queue,
        stop_event: asyncio.Event,
        initial_show_3d: bool
    ) -> None:
        """Main visualization loop running on the event loop thread."""
        current_show_3d = initial_show_3d
        plotter = self._create_plotter() if current_show_3d else None
        last_processed_frame = -1
        latest_packet: Optional[ReconstructionPacket] = None

        try:
            while True:
                if stop_event.is_set() and recon_queue.empty():
                    break

                new_packet: Optional[ReconstructionPacket] = None
                try:
                    while True:
                        new_packet = recon_queue.get_nowait()
                        latest_packet = new_packet
                except asyncio.QueueEmpty:
                    pass

                if new_packet is None:
                    try:
                        latest_packet = await asyncio.wait_for(
                            recon_queue.get(),
                            timeout=self._visual_interval_sec
                        )
                        new_packet = latest_packet
                    except asyncio.TimeoutError:
                        pass

                if new_packet is not None and new_packet.frame.frame_id != last_processed_frame:
                    self._process_visual_packet(new_packet, current_show_3d, plotter)
                    last_processed_frame = new_packet.frame.frame_id

                await asyncio.sleep(self._visual_interval_sec)

                key = cv2.waitKey(1) & 0xFF
                if key in (ord('q'), 27):
                    stop_event.set()
                    break
                if key == ord('s') and latest_packet is not None:
                    await self._save_packet_async(latest_packet)
                if key == ord('v'):
                    current_show_3d = not current_show_3d
                    if current_show_3d:
                ...
                if current_show_3d and plotter is not None and latest_packet is not None and self.move_camera:
                    if latest_packet.hands:
                        center = self.calculate_hands_center(latest_packet.hands)
                    else:
    ...
    def _shutdown_cameras(self) -> None:
        """Stop and release all camera devices."""
        for cam in self.cameras:
    ...
    def reconstruct_hands(
        self,
        frames: List[np.ndarray],
        detections_per_camera: Optional[List[List[HandDetection]]] = None
    ) -> List[Hand3DReconstruction]:
        """Reconstruct hands via hypothesis voting, grouping, and robust fusion."""
        camera_names = list(self.camera_calibrations.keys())

        if detections_per_camera is None:
            detections_per_camera = []
            ...
            # Ensure we work with a shallow copy to avoid mutating shared state
            detections_per_camera = list(detections_per_camera)
            for cam_idx, cam_name in enumerate(camera_names[:len(detections_per_camera)]):
                logger.debug(
                    f"Camera {cam_idx} ({cam_name}): Detected {len(detections_per_camera[cam_idx])} hands"
                )
    ...
    def visualize_2d(
        ...
        for idx, (frame, cam_name) in enumerate(zip(frames, camera_names)):
            ...
            if detections_per_camera is not None and idx < len(detections_per_camera):
                raw_detections = detections_per_camera[idx]
            else:
    ...
    async def run_realtime_async(self, show_3d: bool = True) -> None:
        """Run real-time hand reconstruction using the async pipeline."""
        logger.info("Starting real-time reconstruction (async)...")
        ...
        self._reset_visual_state()
        self.tracked_hands_state.clear()
        self.next_available_hand_id = 0
        self.frame_count = 0
        self.total_reconstructions = 0
        self.camera_angle = 0.0

        frame_queue: asyncio.Queue = asyncio.Queue(maxsize=self._frame_queue_maxsize)
        detection_queue: asyncio.Queue = asyncio.Queue(maxsize=self._detection_queue_maxsize)
        recon_queue: asyncio.Queue = asyncio.Queue(maxsize=self._recon_queue_maxsize)
        stop_event = asyncio.Event()
        ...
        try:
            ...
            self._async_tasks = [
                asyncio.create_task(self._capture_loop(frame_queue, stop_event), name="capture"),
                asyncio.create_task(self._detection_loop(frame_queue, detection_queue, stop_event), name="detect"),
                asyncio.create_task(self._reconstruction_loop(detection_queue, recon_queue, stop_event), name="recon")
            ]

            await self._visualization_loop(recon_queue, stop_event, show_3d)
    ...
    def run_realtime(self, show_3d: bool = True) -> None:
        """Synchronous wrapper that executes the async pipeline."""
        try:
            asyncio.run(self.run_realtime_async(show_3d=show_3d))
